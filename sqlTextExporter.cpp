...
bool sqlTextExporter::export_list(list<op_pid> ids, int utc_offset, double elevation, ostream& out, double& irain_last)
{
   RainMap rain;
   irain_last = 0.0;
   ...
   for(auto& id : ids) {
      ...
      if(ws->is_valid()) {
         ...
         itemp24h.push_back(make_pair(timestamp,ws->itemp()-2.0));

         double ohumi = ws->ohumi();

         out << setw(15) << timestamp << " "
             << setw(15) << ws->itemp()-2.0 << " "
             << setw(15) << ws->ihumi() << " "
             << setw(15) << ws->otemp() << " "
             << setw(15) << ((ohumi>100.0)? 0.0 : ohumi)  << " "
...
void sqlTextExporter::export_numeric_html(list<op_pid> ids, int utc_offset, double elevation, double irain, ostream& out)
{
   ...
   double ohumi = ws->ohumi();
   out << "<td><td>Humidity:</td><td>" << ((ohumi>100.0)? 0.0 : ohumi) << "%</td>" << endl;
   out << "<td><td>Wind-avg.:</td>  <td>" << ws->owspd() << " m/s</td>" << endl;
   out << "</tr>" << endl;

   out << "<tr>" << endl;
   out << "<td>Inside Temp:</td> <td>" << ws->itemp()-2.0 << " &#176C</td>" << endl;
   ...
   out << "<td><td> </td>  <td>" << WSEXPORT_VERSION << " ("<< num_invalid<<") </td>" << endl;
   out << "</tr>" << endl;
